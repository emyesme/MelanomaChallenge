...
def mean_std_masked(image, mask):
...
def get_skewness_kurt_masked(image, mask):
    skewness = []
    kurt = []
    data = image[mask > 0]
    try:
...
def extract_color_features_masked(image, mask):
    color_features = []
    name_features = []

    ## Original image in RGB
    means, stdvs = mean_std_masked(image, mask)
    skewness, kurts = get_skewness_kurt_masked(image, mask)
    color_features.extend(means.reshape(-1))
    color_features.extend(stdvs.reshape(-1))
    ...
    means, stdvs = mean_std_masked(image_hsv, mask)
    skewness, kurts = get_skewness_kurt_masked(image_hsv, mask)
    ...
    means, stdvs = mean_std_masked(image_LAB, mask)
    skewness, kurts = get_skewness_kurt_masked(image_LAB, mask)
    ...
    means, stdvs = mean_std_masked(image_YCC, mask)
    skewness, kurts = get_skewness_kurt_masked(image_YCC, mask)
    ...
    means, stdvs = mean_std_masked(gray_img, mask)
    skewness, kurts =get_skewness_kurt_masked(gray_img, mask)
    ...
    dict_features = {name_features[i]: color_features[i] for i in range(len(name_features))}
